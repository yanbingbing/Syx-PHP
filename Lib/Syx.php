...
/**
 * A quick function to get a Syx_Table Object
 *
...
/**
 * A quick function to make a Syx_Db_Expr
 *
 * @param string $expr
 *
 * @return Syx_Db_Expr
...
abstract class Syx
{
	...
	public static function loadClass($name, $dirs = null)
	{
		...
		if (!class_exists($name, false) && !interface_exists($name, false)) {
			require_once 'Syx/Exception.php';
			throw new Syx_Exception(
	...
	public static function get($index)
	{
		if (!array_key_exists($index, self::$_registry)) {
			require_once 'Syx/Exception.php';
			throw new Syx_Exception("No entry is registered for key '$index'");
		}
	...
	{
		/* register autoloader */
		spl_autoload_register(array('Syx', '__autoload'));

		/* set error handler */
		set_error_handler(array('Syx', '__error_handler'));
		set_exception_handler(array('Syx', '__exception_handler'));
	...
	public static function __exception_handler($e)
	{
		...
		try {
			...
			Syx_Log::log("{$errstr} in {$errfile}:{$errline}", Syx_Log::ERROR);
		} catch (Exception $e) {
		}
		while (ob_get_level() > 0) {
			ob_end_clean();
		}
		require_once 'Syx/Assets/exception.phtml';
		exit;
	}
	...
	public static function __error_handler($errno, $errstr, $errfile, $errline)
	{
		...
		case E_DEPRECATED:
			try {
				require_once 'Syx/Log.php';
				Syx_Log::log("{$errstr} in {$errfile}:{$errline}", Syx_Log::NOTICE);
			} catch (Exception $e) {
			}
			return;
		}
