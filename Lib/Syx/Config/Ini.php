 ...
 * @package   Syx_Config
 */
class Syx_Config_Ini extends Syx_Config
{
		...
		if (strpos($key, '.') !== false) {
			...
			if (strlen($pieces[0]) && strlen($pieces[1])) {
				if (!isset($config[$pieces[0]])) {
					...
					require_once 'Syx/Config/Exception.php';
					throw new Syx_Config_Exception("Cannot create sub-key for '{$pieces[0]}' as key already exists");
				}
				$config[$pieces[0]] = self::_processKey($config[$pieces[0]], $pieces[1], $value);
			} else {
				require_once 'Syx/Config/Exception.php';
				throw new Syx_Config_Exception("Invalid key '$key'");
			}
		} else {
			$config[$key] = $value;
		}
