 ...
 * @package    Syx_Cache
 */
class Syx_Cache_Array extends Syx_Cache_Abstract
{
	...
	protected function _init()
	{
		if (($cacheDir = $this->_config['cacheDir']) != null) {
			if (!$this->_isValidDir($cacheDir)) {
				...
				throw new Syx_Cache_Exception("Unavailable cache dir:$cacheDir");
			}
		} else {
		...
		if (!preg_match('#^[\w]+$#', $this->_config['prefix'])) {
			require_once 'Syx/Cache/Exception.php';
			throw new Syx_Cache_Exception('Invalid filename prefix : must use only [a-zA-A0-9_]');
		}
	...
	protected function _clean($dir, $mode)
	{
		...
		foreach ($glob as $file) {
			if (is_file($file)) {
				switch ($mode) {
				case Syx_Cache::CLEANING_ALL:
					$result = $result && (@unlink($file));
					break;
				case Syx_Cache::CLEANING_OLD:
