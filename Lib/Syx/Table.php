...
class Syx_Table
{
	...
	const DEFAULT_RECORD_CLASS = 'Syx_Table_Record';
	...
	/**
	 * The table column names derived from Syx_Db_Adapter::metaTable().
	 *
	 * @var array
	 */
	...
	{
		if (!is_string($table)) {
			return new Syx_Table($table);
		}
		try {
			$class = $table . 'Table';
			Syx::loadClass($class);
			$instance = new $class();
			if (!$instance instanceof Syx_Table) {
	...
	/**
	 * get Syx_Db_Adapter object the table use
	 *
	 * @return Syx_Db_Adapter
	 */
	...
	public function info($key = null)
	{
		...
		if (!array_key_exists($key, $info)) {
			throw new Syx_Table_Exception("There is no table information for the key '$key'");
		}
	...
	{
		$select = new Syx_Table_Select($this);
		if ($withFrom) {
			$select->from($this);
		}
		return $select;
	...
	public function find()
	{
		...
		$keyc = count($primary);
		if ($argc < $keyc) {
			throw new Syx_Table_Exception("Too few columns for the primary key");
		}
		...
		for ($pos = 0; $pos < $keyc; $pos++) {
			...
			if (!$termc) {
				...
				if (count($arg) != $termc) {
					throw new Syx_Table_Exception('Missing value(s) for the primary key');
				}
			}
			for ($i = 0; $i < $termc; $i++) {
	...
	/**
	 * Fetch a Syx_Table_Rowset object
	 *
	 * @param Syx_Table_Select|string $where
	 * @param string                  $order
	 * @param int                     $count
	 * @param int                     $offset
	 *
	 ...
	 * @param int    $size
	 *
	 * @return array with pagination:Syx_Table_Paginator & rowset:Syx_Table_Rowset
	 */
	public function page($where, $order = null, $page = 1, $size = 20)
	{
		$pagination = new Syx_Table_Paginator(array(
												   'pageSize'    => $size,
												   'currentPage' => $page
											  ));
		...
		return array(
			...
			'rowset'     => new Syx_Table_Rowset($this, $data)
		);
	}
	...
	{
		if (!$this->_adapter) {
			$this->_adapter = Syx_Db::getDefaultAdapter();
		}
		if (!$this->_adapter instanceof Syx_Db_Adapter) {
			throw new Syx_Table_Exception('Need a of type Syx_Db_Adapter object');
		}
	}
	...
	protected function _setupName()
	{
		...
		if (!strlen($this->_name)) {
			throw new Syx_Table_Exception('Needs Table name');
		}
		if (!$this->_quotedName) {
	...
	protected function _setupRecordClass()
	{
		...
		try {
			Syx::loadClass($this->_recordClass);
		} catch (Exception $e) {
			if ($detect) {
				$this->_recordClass = self::DEFAULT_RECORD_CLASS;
				return;
			}
			require_once 'Syx/Table/Exception.php';
			throw new Syx_Table_Exception("Unavailable record class '$this->_recordClass'");
		}
	}
	...
	protected function _setupMetadata()
	{
		if (null === $this->_metadataCache) {
			$this->_metadataCache = Syx_Db::getCacheAdapter();
		}
		...
		if ($this->_metadataCache && !$this->_metadataCache->write($cacheId, $metadata)) {
			throw new Syx_Table_Exception('Failed saving metadata to metadataCache');
		}
		return true;
	}
	...
	protected function _setupPrimaryKey()
	{
		...
		if (!array_intersect((array)$this->_primary, $this->_columns) == (array)$this->_primary) {
			throw new Syx_Table_Exception("Primary key column(s) ("
