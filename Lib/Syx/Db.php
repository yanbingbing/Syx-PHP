 ...
 * @package   Syx_Db
 */
abstract class Syx_Db
{

	/**
	 * Use the CASE_FOLDING constant in the config of a Syx_Db_Adapter.
	 */
	const CASE_FOLDING = 'caseFolding';

	/**
	 * Use the FETCH_MODE constant in the config of a Syx_Db_Adapter.
	 */
	...
	/**
	 * Factory for Syx_Db_Adapter classes.
	 *
	 * @param string $adapter
	 * @param array  $config
	 *
	 * @throws Syx_Db_Exception
	...
	public static function factory($adapter, $config = array())
	{
		...
		if (!is_string($adapter) || empty($adapter)) {
			require_once 'Syx/Db/Exception.php';
			throw new Syx_Db_Exception('Adapter name must be specified in a string');
		}
		...
		if (!is_array($config)) {
			require_once 'Syx/Db/Exception.php';
			throw new Syx_Db_Exception('Adapter parameters must be in an array or a array object');
		}
		/*
		 * Form full adapter class name
		 */
		$adapterNamespace = 'Syx_Db_Adapter';
		...
		$adapterName = Syx::loadClass($adapterNamespace . '_' . strtolower($adapter));
		 ...
		 */
		if (!$dbAdapter instanceof Syx_Db_Adapter) {
			require_once 'Syx/Db/Exception.php';
			throw new Syx_Db_Exception("Adapter class '$adapterName' does not extend Syx_Db_Adapter");
		}
	 ...
	 *
	 * @param Syx_Db_Adapter|string $db
	 *
	 * @return Syx_Db_Adapter
	 * @throws Syx_Db_Exception
	 */
	public static function setDefaultAdapter($db = null)
	{
		...
		if (!$db instanceof Syx_Db_Adapter) {
			require_once 'Syx/Db/Exception.php';
			throw new Syx_Db_Exception(
				'Argument must be of type Syx_Db_Adapter, or a Registry key where a Syx_Db_Adapter object is stored'
			);
		}
	 ...
	 *
	 * @param Syx_Cache_Abstract|string $cache
	 *
	...
	public static function setCacheAdapter($cache = null)
	{
		...
		if (!$cache instanceof Syx_Cache_Abstract) {
			require_once 'Syx/Db/Exception.php';
			throw new Syx_Db_Exception(
				'Argument must be of type Syx_Cache_Abstract, or a Registry key where a Syx_Cache_Abstract object is stored'
			);
		}
