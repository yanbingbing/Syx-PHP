...
abstract class Syx_Db_Adapter
{
	 ...
	 * @var integer
	 */
	protected $_fetchMode = Syx_Db::FETCH_ASSOC;
	 ...
	 * Options
	 * Syx_Db::CASE_NATURAL (default)
	 * Syx_Db::CASE_LOWER
	 * Syx_Db::CASE_UPPER
	 *
	 * @var integer
	 */
	protected $_caseFolding = Syx_Db::CASE_NATURAL;

	/**
	 * Specifies whether the adapter automatically quotes identifiers.
	 * If true, most SQL generated by Syx_Db classes applies
	...
	public function __construct($config)
	{
		...
		if (!is_array($config)) {
			...
			if (is_object($config) && method_exists($config, 'toArray')) {
				...
				throw new Syx_Db_Adapter_Exception('Adapter parameters must be in an array or a object');
			}
		}

		if (isset($config[Syx_Db::CASE_FOLDING])) {
			$case = (int)$config[Syx_Db::CASE_FOLDING];
			switch ($case) {
			case Syx_Db::CASE_LOWER:
			case Syx_Db::CASE_UPPER:
			case Syx_Db::CASE_NATURAL:
				$this->_caseFolding = $case;
				break;
			}
		}

		if (isset($config[Syx_Db::FETCH_MODE])) {
			$mode = $config[Syx_Db::FETCH_MODE];
			if (is_string($mode)) {
				$constant = 'Syx_Db::FETCH_' . strtoupper($mode);
				if (defined($constant)) {
		...
		foreach ($this->_options as $key => $optional) {
			if (isset($config[$key])) {
				...
				require_once 'Syx/Db/Adapter/Exception.php';
				throw new Syx_Db_Adapter_Exception(
	...
	protected function _connect()
	{
		...
		if (!extension_loaded('pdo')) {
			require_once 'Syx/Db/Adapter/Exception.php';
			throw new Syx_Db_Adapter_Exception('The PDO extension is required for this adapter but the extension is not loaded');
		}

		// check the PDO driver is available
		if (!in_array($this->_driver, PDO::getAvailableDrivers())) {
			require_once 'Syx/Db/Adapter/Exception.php';
			throw new Syx_Db_Adapter_Exception('The ' . $this->_driver . ' driver is not currently installed');
		}
		...
		try {
			...
			require_once 'Syx/Db/Adapter/Exception.php';
			throw new Syx_Db_Adapter_Exception($e->getMessage(), $e->getCode());
		}
	}
	...
	{
		return new Syx_Db_Select($this);
	}

	/**
	 * Prepares and executes an SQL statement with bound data.
	 *
	 * @param string|Syx_Db_Select $statement
	 *
	 * @throws Syx_Db_Adapter_Exception
	 * @return int
	 */
	...
	{
		if ($statement instanceof Syx_Db_Select) {
		...
		try {
			...
			if ($affected === false) {
				...
				require_once 'Syx/Db/Adapter/Exception.php';
				throw new Syx_Db_Adapter_Exception($errorInfo[2]);
			}

			return $affected;
		} catch (Syx_Db_Statement_Exception $e) {
			require_once 'Syx/Db/Adapter/Exception.php';
			throw new Syx_Db_Adapter_Exception($e->getMessage(), $e->getCode());
		}
	}
	 ...
	 * Prepares and executes an SQL statement with bound data.
	 *
	 * @param  string|Syx_Db_Select $sql  The SQL statement with placeholders.
	...
	{
		// is the $sql a Syx_Db_Select object?
		if ($sql instanceof Syx_Db_Select) {
			if (empty($bind)) {
				$bind = $sql->getBind();
			}
	 ...
	 *
	 * @param string|Syx_Db_Select  $sql       An SQL SELECT statement.
	...
	public function fetchAssoc($sql, $bind = array())
	{
		$stmt = $this->query($sql, $bind);
		$data = array();
		while (($row = $stmt->fetch(Syx_Db::FETCH_ASSOC)) != false) {
	...
	{
		$stmt = $this->query($sql, $bind);
		$result = $stmt->fetchAll(Syx_Db::FETCH_COLUMN, 0);
		return $result;
	}
	...
	public function fetchPairs($sql, $bind = array())
	{
		$stmt = $this->query($sql, $bind);
		$data = array();
		while (($row = $stmt->fetch(Syx_Db::FETCH_NUM)) != false) {
			$data[$row[0]] = $row[1];
		}
		return $data;
	}
	 ...
	 *
	 * @param mixed|Syx_Db_Select|Syx_Db_Expr $value The value to quote.
	...
	public function quote($value, $type = null)
	{
		...
		if ($value instanceof Syx_Db_Select) {
		...
		if ($type !== null && array_key_exists($type = strtoupper($type), $this->_numericDataTypes)) {
			...
			case Syx_Db::INT_TYPE: // 32-bit integer
				$quotedValue = (string)intval($value);
				break;
			case Syx_Db::BIGINT_TYPE: // 64-bit integer
				...
				break;
			case Syx_Db::FLOAT_TYPE: // float or decimal
	 ...
	 * the adapter.
	 *
	 * @param string|array|Syx_Db_Expr $ident The identifier.
	...
	{
		if ($ident instanceof Syx_Db_Expr) {
			...
			if ($table instanceof Syx_Db_Expr) {
	 ...
	 *
	 * @param string|array|Syx_Db_Expr|Syx_Db_Select $ident The identifier or expression.
	...
	protected function _quoteIdentifierAs($ident, $alias = null, $auto = false)
	{
		if ($ident instanceof Syx_Db_Expr) {
			$quoted = $ident->__toString();
		} elseif ($ident instanceof Syx_Db_Select) {
			...
			if (is_array($ident)) {
				...
				foreach ($ident as $segment) {
					if ($segment instanceof Syx_Db_Expr) {
	...
	{
		$stmt = new Syx_Db_Statement($this, $sql);
		$stmt->setFetchMode($this->_fetchMode);
		return $stmt;
	}
	...
	{
		switch ($mode) {
		case Syx_Db::FETCH_LAZY:
		case Syx_Db::FETCH_ASSOC:
		case Syx_Db::FETCH_NUM:
		case Syx_Db::FETCH_BOTH:
		case Syx_Db::FETCH_NAMED:
		case Syx_Db::FETCH_OBJ:
			$this->_fetchMode = $mode;
			break;
		default:
			require_once 'Syx/Db/Adapter/Exception.php';
			throw new Syx_Db_Adapter_Exception("Invalid fetch mode '$mode' specified");
			break;
		}
	}
	...
	{
		return $this->isConnected() ? $this->_connection->errorInfo() : array(0 => Syx_Db::ERR_NONE);
	}
	...
	{
		return $this->isConnected() ? $this->_connection->errorCode() : Syx_Db::ERR_NONE;
	}
