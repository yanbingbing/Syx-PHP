...
class Syx_Db_Select
{
	...
	/**
	 * Syx_Db_Adapter object.
	 *
	 * @var Syx_Db_Adapter
	 */
	protected $_adapter;

	public function __construct(Syx_Db_Adapter $adapter)
	{
	...
	/**
	 * Gets the Syx_Db_Adapter for this
	 * particular Syx_Db_Select object.
	 *
	...
	public function getPart($part)
	{
		if (!array_key_exists($part, $this->_parts)) {
			...
			throw new Syx_Db_Exception("Invalid Select part '$part'");
		}
	...
	public function columns($cols = '*', $correlationName = null)
	{
		...
		if (!array_key_exists($correlationName, $this->_parts[self::FROM])) {
			require_once 'Syx/Db/Exception.php';
			throw new Syx_Db_Exception("No table has been specified for the FROM clause");
		}
	...
	public function join($type, $table, $on, $columns = '*')
	{
		if (!isset(self::$_joinTypes[$type])) {
			require_once 'Syx/Db/Exception.php';
			throw new Syx_Db_Exception("Invalid join type '$type'");
		}
	...
	{
		if ($columns instanceof Syx_Db_Expr) {
	...
	protected function _renderColumns($sql)
	{
		...
		foreach ($this->_parts[self::COLUMNS] as $columnEntry) {
			...
			if ($column instanceof Syx_Db_Expr) {
				$columns[] = $this->_adapter->quoteColumnAs($column, $alias, true);
			} else {
				if ($column == self::SQL_WILDCARD) {
					$column = new Syx_Db_Expr(self::SQL_WILDCARD);
					$alias = null;
				}
