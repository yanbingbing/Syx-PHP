 ...
 * @package    Syx_Table
 */
class Syx_Table_Record implements ArrayAccess, IteratorAggregate
{
	const ATTR_COLUMN = 0;
	 ...
	 * for internal use
	 *
	 * @param Syx_Table $table
	 * @param array     $data
	 */
	final public function __construct(Syx_Table $table, array $data = array())
	{
		$this->_table = $table;
		$info = $table->info();
		$this->_columns = array_flip($info[Syx_Table::COLUMNS]);
		$this->_primary = $info[Syx_Table::PRIMARY];
		if (!empty($data)) {
	 ...
	 *
	 * @return array|mixed
	 * @throws Syx_Table_Record_Exception
	 */
	protected function _insert()
	{
		...
		if (!$this->_validate(self::ON_INSERT)) {
			require_once 'Syx/Table/Record/Exception.php';
			throw new Syx_Table_Record_Exception('Validation failure');
		}
	...
	protected function _getPkData($useDirty = true)
	{
		...
		if (count($primary) != count($pkData)) {
			require_once 'Syx/Table/Record/Exception.php';
			throw new Syx_Table_Record_Exception("primary data is not set enough");
		}
		return $pkData;
	}
	...
	public function setDefaultValue($column, $data, $on = null)
	{
		if (!array_key_exists($column, $this->_columns)) {
			require_once 'Syx/Table/Record/Exception.php';
			throw new Syx_Table_Record_Exception("Column name '$column' is not found");
		}
		if ($this->_isNull($data)) {
			require_once 'Syx/Table/Record/Exception.php';
			throw new Syx_Table_Record_Exception("Null data for column '$column' was given");
		}
		if (!($on & self::ON_BOTH)) {
			$on = self::ON_BOTH;
		}
	 ...
	 *
	 * @throws Syx_Table_Record_Exception
	 * @return Syx_Table_Validator_Interface
	 */
	protected function _retrieveValidator($name)
	{
		if ($name instanceof Syx_Table_Validator_Interface) {
			return $name;
		}

		if (!is_string($name)) {
			throw new Syx_Table_Record_Exception('Invalid validator');
		}
		...
		if (preg_match('/^\w+$/', $name)) {
			...
			try {
				...
				if (!$rf->isSubclassOf('Syx_Table_Validator_Interface')) {
					throw new Exception('catch');
				}
			} catch (Exception $e) {
				$class = 'Syx_Table_Validator_' . $name;
				Syx::loadClass($class);
			}
			$inst = new $class();
		} else {
			$inst = new Syx_Table_Validator_Regex($name);
		}
	...
	protected function _validate($on, $breakOnFailure = false)
	{
		...
		foreach ($columns as $column => $v) {
			...
			if ($value instanceof Syx_Db_Expr) {
				continue;
			} else {
				if (is_object($value)) {
					$value = (string)$value;
	...
	public function refresh()
	{
		...
		if (empty($this->_data)) {
			require_once 'Syx/Table/Record/Exception.php';
			throw new Syx_Table_Record_Exception('Cannot refresh record as it is missing');
		}
		$this->_cleanData = $this->_data;
		return $this;
	}
