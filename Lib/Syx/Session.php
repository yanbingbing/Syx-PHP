...
abstract class Syx_Session
{
	...
	public static function setOptions(array $userOptions = array())
	{
		...
		foreach ($userOptions as $userOptionName => $userOptionValue) {
			...
			if (array_key_exists($userOptionName, self::$_defaultOptions)) {
				...
				require_once 'Syx/Session/Exception.php';
				throw new Syx_Session_Exception("Unknown option: $userOptionName = $userOptionValue");
			}
		}
	}
	 ...
	 *
	 * @param Syx_Session_SaveHandler $saveHandler
	 */
	public static function setSaveHandler(Syx_Session_SaveHandler $saveHandler)
	{
	 ...
	 * get saveHandler
	 *
	 * @return Syx_Session_SaveHandler
	 */
	 ...
	 * @param bool $options
	 *
	 * @throws Syx_Session_Exception
	 */
	public static function start($options = false)
	{
		if (self::$_sessionStarted && self::$_destroyed) {
			require_once 'Syx/Session/Exception.php';
			throw new Syx_Session_Exception('The session was explicitly destroyed during this request.');
		}
		...
		if (headers_sent($filename, $linenum)) {
			require_once 'Syx/Session/Exception.php';
			throw new Syx_Session_Exception("Session must be started before any output has been sent to the browser;"
				. " output started in {$filename}/{$linenum}");
		}

		if (defined('SID')) {
			require_once 'Syx/Session/Exception.php';
			throw new Syx_Session_Exception('session has already been started by session.auto-start or session_start()');
		}
		...
		try {
			...
			session_write_close();
			require_once 'Syx/Session/Exception.php';
			throw new Syx_Session_Exception(__CLASS__ . '::' . __FUNCTION__ . '() - ' . $e->getMessage());
		}
	...
	public static function setId($id)
	{
		if (defined('SID')) {
			require_once 'Syx/Session/Exception.php';
			throw new Syx_Session_Exception('The session has already been started.  The session id must be set first.');
		}

		$filename = $linenum = null;
		if (headers_sent($filename, $linenum)) {
			require_once 'Syx/Session/Exception.php';
			throw new Syx_Session_Exception("You must call " . __CLASS__ . '::' . __FUNCTION__ .
				"() before any output has been sent to the browser; output started in {$filename}/{$linenum}");
		}

		if (!is_string($id) || $id === '') {
			require_once 'Syx/Session/Exception.php';
			throw new Syx_Session_Exception('You must provide a non-empty string as a session identifier.');
		}
